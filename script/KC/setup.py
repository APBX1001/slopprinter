...
def _args():
    ...
    L = ['Forge-Classic', 'Forge-Neo', 'SwarmUI']

    parser = argparse.ArgumentParser(description='WebUI Installer Script for Kaggle and Google Colab')
    parser.add_argument('--webui', required=True, help='available webui: Forge-Classic, Forge-Neo, SwarmUI')
...
def _reqs(W, M):
    ...
    u = M / 'upscale_models' if ui == 'SwarmUI' else M / 'ESRGAN'
...
def _setup():
    ...
    E = M / 'Embeddings' if ui == 'SwarmUI' else (M / 'embeddings' if ui in ['Forge-Classic', 'Forge-Neo'] else WEBUI / 'embeddings')
    V = M / 'VAE'
